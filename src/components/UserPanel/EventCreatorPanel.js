...
import { Select, MenuItem, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Stack } from "@mui/material";
...
const EventCreatorPanel = () => {
  ...
  const fetchData = async (table) => {
    ...
    try {
      ...
      if (array) {
        console.log("event creator response data");
        console.log(array);
  ...
  return (
    <section>
      ...
      <Stack 
        ...
        sx={{ marginBottom: 2 }}
      >
        <Select value={selectedTable} onChange={(e) => setSelectedTable(e.target.value)} displayEmpty sx={{ minWidth: 200 }}>
        ...
        {selectedTable == "Events" ? (
            <Button variant="contained" className="btn btn-primary">Dodaj wydarzenie</Button>
          ) : null}
          {selectedTable == "EventTickets" ? (
            <Button variant="contained" className="btn btn-primary">Dodaj pulę biletów</Button>
          ) : null}
